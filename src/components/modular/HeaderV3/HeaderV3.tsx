    ...
    <>
      <header className="sticky top-0 z-[60] w-full bg-white/80 backdrop-blur-md px-5 md:px-8 lg:px-12 py-6 flex items-center justify-between transition-colors duration-300">
        ...
        <div className="flex-1 flex items-center gap-6 md:gap-8">
          ...
          >
            <span className={`w-5 h-[1.5px] bg-black block transition-all duration-300 ${isMenuOpen ? 'rotate-45 translate-y-[6.5px]' : 'group-hover:w-6'}`}></span>
            <span className={`w-3.5 h-[1.5px] bg-black block transition-all duration-300 ${isMenuOpen ? 'opacity-0' : 'group-hover:w-5'}`}></span>
            <span className={`w-5 h-[1.5px] bg-black block transition-all duration-300 ${isMenuOpen ? '-rotate-45 -translate-y-[6.5px]' : 'group-hover:w-6'}`}></span>
          </button>
        ...
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex justify-center items-center z-[60]">
          <Link href="/" aria-label="Home" className="group" onClick={() => setIsMenuOpen(false)}>
            <svg 
              width="28" 
              height="28" 
              viewBox="0 0 32 32" 
      ...
      >
        <div className="max-w-4xl w-full mx-auto flex flex-col gap-10 md:gap-16 pt-32 pb-20">
          ...
          {/* Search Box */}
          <div className={`flex items-center border-b-2 border-black/10 pb-4 group transition-all duration-700 delay-100 flex-shrink-0 ${isMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
